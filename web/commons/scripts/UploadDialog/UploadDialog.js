...
//									});
	...
	onUploadButtonClick : function() {
		if(Ext.getCmp('errorInfoPanel')){
			Ext.getCmp('errorInfoPanel').hide(true);
			Ext.getCmp('errorInfo').setValue('');
		}
		if (this.is_uploading) {
	...
	onAjaxSuccess : function(response, options) {
		...
		if ("success" in json_response && json_response.success) {
			...
			if(json_response.info != '' && json_response.info != null && typeof json_response.info != 'undefined'){
				var info = json_response.info;
				var obj = Ext.getCmp('FileUploadImport') || Ext.getCmp('FileUploadExport');
				var errorWindow = Ext.getCmp('errorInfoPanel');
				Ext.getCmp('errorInfo').setValue(json_response.info.replace(/\hh/g,'\n'));
				errorWindow.setSize(350,obj.height);
				errorWindow.setPosition(obj.x+obj.width,obj.y);
